...
def get_transaction(userId):
    ...
    dataset['amount_in_dollars'] = round(dataset.loc[:, 'amount_cents'] / 100, 0)
    dataset['amount_in_dollars'] = dataset.loc[:,'amount_in_dollars'].astype(int)
    dataset['datetime'] = dataset.loc[:, 'datetime'].dt.strftime('%Y-%m-%dT%H:%M:%S+00:00')
...
def get_net_return(merchantTypeCode):
    ...
    all_data['date'] = all_data.loc[:, 'datetime'].dt.strftime('%Y-%m-%d')
    ...
    result_data['amount_cents_purchase'] = result_data.loc[:, 'amount_cents_purchase'].fillna(0)
    result_data['amount_cents_return'] = result_data.loc[:, 'amount_cents_return'].fillna(0)
    result_data['net_amount_in_dollars'] = round((result_data.loc[:, 'amount_cents_purchase'] - result_data['amount_cents_return']) / 100, 0)
    result_data['net_amount_in_dollars'] = result_data['net_amount_in_dollars'].astype(int)
    result_data['merchant_type_code'] = int(merchantTypeCode)
